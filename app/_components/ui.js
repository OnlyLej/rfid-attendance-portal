    ...
    <div
      ...
      style={{
        backgroundImage: darkMode
          ...
          : 'linear-gradient(90deg,#f1f5f9 0%,#e8edf5 50%,#f1f5f9 100%)',
      ...
      <div className="flex items-end gap-2 flex-1">
        {bars.map((h, i) => (
          <div key={i} className="flex-1 flex flex-col justify-end gap-1">
            <Skeleton darkMode={darkMode} className="w-full rounded-t-lg" style={{ height: `${h * 100}%` }} />
          </div>
        ))}
      </div>
      <div className="flex gap-2">
        {bars.map((_, i) => (
          <Skeleton key={i} darkMode={darkMode} className="flex-1 h-2.5 rounded-full" />
        ))}
      </div>
    </div>
  );

...
const Card = ({ children, className = '', darkMode, delay = 0, hover = false }) => {
  ...
  return (
    <div className={`${darkMode ? 'bg-gray-800/70 border-gray-700/60' : 'bg-white border-gray-200/80 shadow-[0_1px_4px_rgba(0,0,0,0.06),0_0_0_1px_rgba(0,0,0,0.03)]'} border rounded-2xl backdrop-blur-sm transition-all duration-500 ease-out ${hover ? (darkMode ? 'hover:bg-gray-800 hover:border-gray-600 hover:shadow-lg hover:-translate-y-0.5' : 'hover:border-gray-300 hover:shadow-[0_4px_16px_rgba(0,0,0,0.08)] hover:-translate-y-0.5') : ''} ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'} ${className}`}>
      {children}
    </div>
  );
};
...
const StatCard = ({ icon: Icon, label, value, color, darkMode, delay = 0, numericValue = null, trend = null, trendLabel = '' }) => {
  ...
  const colorMap = {
    green:  { dark: { bg: 'bg-emerald-500/10', text: 'text-emerald-400' }, light: { bg: 'bg-emerald-50', text: 'text-emerald-600' } },
    red:    { dark: { bg: 'bg-rose-500/10',    text: 'text-rose-400' },    light: { bg: 'bg-rose-50',    text: 'text-rose-600' } },
    blue:   { dark: { bg: 'bg-sky-500/10',     text: 'text-sky-400' },     light: { bg: 'bg-sky-50',     text: 'text-sky-600' } },
    purple: { dark: { bg: 'bg-violet-500/10',  text: 'text-violet-400' },  light: { bg: 'bg-violet-50',  text: 'text-violet-600' } },
    indigo: { dark: { bg: 'bg-indigo-500/10',  text: 'text-indigo-400' },  light: { bg: 'bg-indigo-50',  text: 'text-indigo-600' } },
    orange: { dark: { bg: 'bg-amber-500/10',   text: 'text-amber-400' },   light: { bg: 'bg-amber-50',   text: 'text-amber-600' } },
  };
  ...
  const gradientTop = color === 'green' ? 'from-emerald-400 to-emerald-600' : color === 'red' ? 'from-rose-400 to-rose-600' : color === 'blue' ? 'from-sky-400 to-sky-600' : color === 'purple' ? 'from-violet-400 to-violet-600' : color === 'indigo' ? 'from-indigo-400 to-indigo-600' : 'from-amber-400 to-amber-600';

  return (
    <div ref={ref} className={`${darkMode ? 'bg-gray-800/80 border-gray-700/60 hover:bg-gray-800 hover:border-gray-500' : 'bg-white border-gray-200/80 hover:border-gray-300 shadow-[0_1px_6px_rgba(0,0,0,0.05)] hover:shadow-[0_4px_20px_rgba(0,0,0,0.09)]'} border rounded-2xl p-5 backdrop-blur-sm transition-all duration-500 ease-out hover:-translate-y-1 cursor-default ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'} group relative overflow-hidden`} style={{ transitionDelay: `${delay}ms` }}>
      <div className={`absolute top-0 left-0 right-0 h-0.5 rounded-t-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-r ${gradientTop}`} />
      <div className="flex items-start justify-between mb-4">
        <div className={`inline-flex p-2.5 rounded-xl ${iconBg} transition-transform duration-300 group-hover:scale-110`}><Icon size={18} className={iconText} /></div>
        {trend !== null && <div className={`flex items-center gap-0.5 text-xs font-medium ${trendColor}`}><TrendIcon size={12} /><span>{Math.abs(trend)}%</span></div>}
      </div>
...
const Skeleton = ({ className = '', darkMode }) => (
  <div className={`rounded-xl overflow-hidden ${className}`}><div className={`w-full h-full ${darkMode ? 'bg-gray-700/50' : 'bg-gray-200/60'} skeleton-pulse`} /></div>
);
  ...
  <div className="flex flex-col gap-3 h-full justify-end pb-2">
    <div className="flex items-end gap-2 h-full">
      {[0.4, 0.7, 0.5, 0.9, 0.6, 0.8, 0.5].map((h, i) => (
        <div key={i} className="flex-1"><div className={`w-full rounded-t-lg skeleton-pulse ${darkMode ? 'bg-gray-700/60' : 'bg-gray-200/60'}`} style={{ height: `${h * 100}%` }} /></div>
      ))}
    </div>
  </div>
);
...
const ChartHeader = ({ icon: Icon, title, badge, color = 'sky', darkMode }) => {
  const iconColors = { sky: 'bg-sky-500/10 text-sky-500', emerald: 'bg-emerald-500/10 text-emerald-500', violet: 'bg-violet-500/10 text-violet-500', indigo: 'bg-indigo-500/10 text-indigo-500', amber: 'bg-amber-500/10 text-amber-500' };
  return (
    <div className="flex items-center justify-between mb-5">
      <div className="flex items-center gap-3">
        <div className={`w-8 h-8 rounded-xl ${iconColors[color]} flex items-center justify-center`}><Icon size={15} /></div>
        <h3 className={`text-sm font-semibold ${darkMode ? 'text-white' : 'text-gray-800'}`}>{title}</h3>
      </div>
      {badge && <span className={`text-xs px-2.5 py-1 rounded-full font-medium border ${darkMode ? 'bg-gray-700/60 border-gray-600 text-gray-400' : 'bg-gray-50 border-gray-200 text-gray-500'}`}>{badge}</span>}
    </div>
  );
};
...
export default function DashboardTab({ darkMode, stats, weekData, students, logs, classes, loading }) {
  ...
  const classComparisonData = useMemo(() => {
    ...
    return classes.map(cls => {
      ...
      classStudents.forEach(student => {
        ...
        const studentLogs = (todayLogsByStudent.get(nid) || []).slice().sort((a, b) => { const da = parsePhTimestamp(a.timestamp); const db = parsePhTimestamp(b.timestamp); return (da?.getTime() ?? 0) - (db?.getTime() ?? 0); });
        if (!studentLogs.length) { noLogCount++; } else { studentLogs[studentLogs.length - 1].status === 'IN' ? presentCount++ : absentCount++; }
      });
  ...
  const tooltipStyle = { backgroundColor: darkMode ? '#1e293b' : '#ffffff', border: `1px solid ${darkMode ? '#334155' : '#f1f5f9'}`, borderRadius: '12px', boxShadow: '0 8px 32px rgba(0,0,0,0.18)', color: darkMode ? '#e2e8f0' : '#1e293b', fontSize: 12, padding: '10px 14px' };
  ...
  return (
    <div className="space-y-5">
      {loading && (
        <div className={`flex items-center gap-3 px-4 py-3 rounded-2xl border text-sm font-medium ${darkMode ? 'bg-sky-900/20 border-sky-800/40 text-sky-400' : 'bg-sky-50 border-sky-100 text-sky-600'}`}>
          <div className="flex gap-1">{[0,1,2].map(i => <div key={i} className="w-1.5 h-1.5 rounded-full bg-sky-500 animate-bounce" style={{ animationDelay: `${i * 0.12}s` }} />)}</div>
        ...
        <Card darkMode={darkMode} delay={100} hover>
          <div className="p-5">
            ...
            <div style={{ height: chartH }}>
              {loading ? <ChartSkeleton darkMode={darkMode} /> : weeklyData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={weeklyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }} barCategoryGap="35%">
                    <defs>
                      <linearGradient id="presentGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={1}/><stop offset="95%" stopColor="#059669" stopOpacity={0.85}/></linearGradient>
                      <linearGradient id="absentGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#f43f5e" stopOpacity={1}/><stop offset="95%" stopColor="#e11d48" stopOpacity={0.85}/></linearGradient>
                    </defs>
              ...
              ) : <div className="h-full flex flex-col items-center justify-center gap-2"><BarChart3 size={28} className={darkMode ? 'text-gray-700' : 'text-gray-200'} /><p className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>No data for this month</p></div>}
            </div>
          </div>
        </Card>
        ...
        <Card darkMode={darkMode} delay={150} hover>
          <div className="p-5">
            ...
            <div style={{ height: chartH }}>
              {loading ? <ChartSkeleton darkMode={darkMode} /> : dailyData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={dailyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                    <defs>
                      <linearGradient id="dailyPresentGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={0.3}/><stop offset="95%" stopColor="#10b981" stopOpacity={0}/></linearGradient>
                      <linearGradient id="dailyAbsentGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#f43f5e" stopOpacity={0.2}/><stop offset="95%" stopColor="#f43f5e" stopOpacity={0}/></linearGradient>
                    </defs>
                    <CartesianGrid strokeDasharray="3 3" stroke={gridColor} vertical={false} />
                    <XAxis dataKey="name" stroke={axisColor} tick={{ fontSize: 11, fontWeight: 500 }} tickLine={false} axisLine={false} />
                    <YAxis stroke={axisColor} tick={{ fontSize: 11 }} tickLine={false} axisLine={false} />
              ...
              ) : <div className="h-full flex flex-col items-center justify-center gap-2"><Activity size={28} className={darkMode ? 'text-gray-700' : 'text-gray-200'} /><p className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>No data yet</p></div>}
            </div>
          </div>
        </Card>
        ...
        <Card darkMode={darkMode} delay={200} hover>
          <div className="p-5">
            ...
            <div style={{ height: chartH }}>
              {loading ? <ChartSkeleton darkMode={darkMode} /> : monthlyData.some(m => m.avgPresent !== null && m.avgPresent > 0) ? (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={monthlyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                    <defs>
                      <linearGradient id="monthlyGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#8b5cf6" stopOpacity={0.35}/><stop offset="95%" stopColor="#8b5cf6" stopOpacity={0}/></linearGradient>
                      <linearGradient id="monthlyRateGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#06b6d4" stopOpacity={0.25}/><stop offset="95%" stopColor="#06b6d4" stopOpacity={0}/></linearGradient>
                    </defs>
                    <CartesianGrid strokeDasharray="3 3" stroke={gridColor} vertical={false} />
              ...
              ) : <div className="h-full flex flex-col items-center justify-center gap-2"><TrendingUp size={28} className={darkMode ? 'text-gray-700' : 'text-gray-200'} /><p className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>No monthly data yet</p></div>}
            </div>
          </div>
        </Card>
        ...
        <Card darkMode={darkMode} delay={250} hover>
          <div className="p-5">
            ...
            <div style={{ height: chartH }}>
              {loading ? <ChartSkeleton darkMode={darkMode} /> : classComparisonData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={classComparisonData} layout="vertical" margin={{ top: 4, right: 50, left: 8, bottom: 0 }}>
                    <defs><linearGradient id="classGrad" x1="0" y1="0" x2="1" y2="0"><stop offset="5%" stopColor="#6366f1" stopOpacity={1}/><stop offset="95%" stopColor="#8b5cf6" stopOpacity={0.85}/></linearGradient></defs>
              ...
              ) : <div className="h-full flex flex-col items-center justify-center gap-2"><Target size={28} className={darkMode ? 'text-gray-700' : 'text-gray-200'} /><p className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>No class data today</p></div>}
            </div>
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { title: "Today's Summary", icon: Calendar, iconColor: 'text-sky-500', iconBg: 'bg-sky-500/10', content: (
            <div className="space-y-3">
              {[{ label: 'Check-ins (IN)', value: stats.presentToday }, { label: 'Checked Out (OUT)', value: stats.absentToday, red: true }, { label: 'Attendance Rate', value: `${stats.attendanceRate}%`, green: true }].map((r, i) => (
          ...
          { title: 'Week Snapshot', icon: TrendingUp, iconColor: 'text-emerald-500', iconBg: 'bg-emerald-500/10', content: (
            <div className="space-y-3">
              {[{ label: 'Avg Daily Present', value: dailyData.length > 0 ? Math.round(dailyData.reduce((s,d) => s+d.present, 0) / dailyData.length) : 0 }, { label: 'Avg Rate', value: `${weekAvg}%`, purple: true }, { label: 'Best Day', value: dailyData.length > 0 && dailyData.some(d => d.present > 0) ? dailyData.reduce((m,d) => d.present > m.present ? d : m, dailyData[0]).name : '—', green: true }].map((r, i) => (
                <div key={i} className={`flex justify-between items-center py-2 border-b last:border-0 ${darkMode ? 'border-gray-700/50' : 'border-gray-100'}`}>
                  <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{r.label}</span>
          ...
          { title: 'Top Class Today', icon: Award, iconColor: 'text-amber-500', iconBg: 'bg-amber-500/10', content: classComparisonData.length > 0 ? (
            <div>
              ...
              {classComparisonData.length > 1 && <p className={`text-xs truncate ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>2nd: {classComparisonData[1].fullName} — {classComparisonData[1].attendanceRate}%</p>}
            </div>
          ) : <p className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>No data yet</p>, delay: 400 },
        ].map((card, i) => (
          <Card key={i} darkMode={darkMode} delay={card.delay} hover>
            <div className="p-5">
              <div className="flex items-center gap-2.5 mb-4">
                <div className={`w-8 h-8 rounded-xl ${card.iconBg} flex items-center justify-center`}><card.icon size={15} className={card.iconColor} /></div>

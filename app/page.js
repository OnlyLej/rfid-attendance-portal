...
const DashboardTab = ({ darkMode, stats, weekData, students, logs, classes, loading }) => {
  ...
  const classComparisonData = useMemo(() => {
    ...
    const todayLogsByStudent = new Map<string, { status: string }[]>();
    logs.forEach(log => {
              ...
              <tbody className={`divide-y ${darkMode ? 'divide-gray-700/40' : 'divide-gray-50'}`}>
                {pagedLogs.map((log, i) => (
                  <tr key={i} className={`transition-colors duration-100 group ${darkMode ? 'hover:bg-gray-700/30' : 'hover:bg-slate-50/80'}`}>
                    ...
                    <td className={`px-5 py-3.5 text-sm font-semibold max-w-[180px]`}>
                      <span className={`block truncate ${darkMode ? 'text-white' : 'text-gray-800'}`} title={log.name}>{log.name}</span>
                    </td>
                    <td className={`px-5 py-3.5 text-sm max-w-[140px]`}>
...
// Drop-in replacement for the LandingPage component in page.tsx
...
const LandingPage = ({ darkMode, toggleTheme, onLogin, animatedNumbers }) => {
  ...
  useEffect(() => {
    const tick = setInterval(() => {
      setLiveStats(prev => {
        // Gently drift each value within realistic bounds
        const rand = (base, spread) => Math.max(0, base + Math.round((Math.random() - 0.5) * spread));
        const students = rand(prev.students, 4);
        const present  = Math.min(students, rand(prev.present, 5));
        const absent   = students - present;
        ...
        // Track which fields changed so we can flash them
        const changed = {};
        if (students   !== prev.students)   changed.students = true;
        if (present    !== prev.present)    changed.present  = true;
        if (absent     !== prev.absent)     changed.absent   = true;
  ...
  // Flash style for stat numbers
  const flashCls = (key) => statFlash[key]
    ? 'scale-110 text-sky-400 transition-all duration-150'
    : 'transition-all duration-300';

  return (
    <div className={`min-h-screen overflow-x-hidden ${darkMode ? 'bg-[#080c14] text-white' : 'bg-[#f7f9fc] text-gray-900'} transition-colors duration-300`}>

      {/* Ambient background blobs that follow mouse */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden" style={{ zIndex: 0 }}>
        <div
          className={`absolute w-[600px] h-[600px] rounded-full blur-[120px] transition-[left,top] duration-1000 ease-out ${darkMode ? 'bg-sky-900/25' : 'bg-sky-200/50'}`}
          style={{ left: `${-10 + px * 15}%`, top: `${-10 + py * 10}%` }}
        />
        <div
          className={`absolute w-[500px] h-[500px] rounded-full blur-[100px] transition-[right,bottom] duration-[1400ms] ease-out ${darkMode ? 'bg-violet-900/20' : 'bg-violet-100/60'}`}
          style={{ right: `${-5 + (1 - px) * 12}%`, bottom: `${10 + py * 8}%` }}
        />
        <div
          className={`absolute w-[300px] h-[300px] rounded-full blur-[80px] ${darkMode ? 'bg-emerald-900/15' : 'bg-emerald-100/40'}`}
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowModal(true)}
              className="relative px-4 py-2 text-sm font-bold text-white rounded-xl overflow-hidden group transition-all duration-200 hover:scale-105 active:scale-95 shadow-lg shadow-sky-500/30"
      ...
      <section ref={heroRef} className="relative max-w-6xl mx-auto px-4 sm:px-6 pt-20 pb-28 md:pt-28 md:pb-36" style={{ zIndex: 1 }}>
        ...
        <div className="text-center space-y-6 max-w-4xl mx-auto">
          <h1 className="text-5xl md:text-7xl font-black leading-[0.95] tracking-tighter animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
            ...
            <br className="hidden md:block" />
            <span className="relative inline-block">
              <span style={{ background: 'linear-gradient(135deg, #0ea5e9 0%, #7c3aed 50%, #10b981 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                Automated.
              </span>
              <svg className="absolute -bottom-2 left-0 w-full" height="6" viewBox="0 0 200 6" preserveAspectRatio="none">
                <path d="M0 5 Q50 0 100 4 Q150 8 200 3" stroke="url(#heroUnderlineGrad)" strokeWidth="2.5" fill="none" strokeLinecap="round" />
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
            <button onClick={() => setShowModal(true)}
              className="group relative px-7 py-3.5 text-sm font-bold text-white rounded-xl overflow-hidden transition-all duration-200 hover:scale-105 active:scale-95 shadow-xl shadow-sky-500/30 flex items-center gap-2"
            ...
            <button
              ...
              className={`px-7 py-3.5 text-sm font-bold rounded-xl border transition-all duration-200 hover:scale-105 active:scale-95 flex items-center gap-2
                ${darkMode ? 'border-white/10 text-gray-300 hover:bg-white/5' : 'border-gray-200 text-gray-700 hover:bg-gray-100'}`}>
              See how it works <ChevronDown size={16} />
            </button>
          </div>
        </div>

        {/* ── LIVE stat counters — randomize every second ── */}
        <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 animate-fade-in-up" style={{ animationDelay: '0.45s' }}>
          {[
            { key: 'students',     val: `${liveStats.students}`,        label: 'Students Enrolled',  icon: Users,        accent: '#0ea5e9' },
            { key: 'checkins',     val: `${liveStats.checkins}+`,       label: 'Today\'s Check-ins', icon: CheckCircle,  accent: '#10b981' },
            ...
            <div key={i}
              className={`group relative overflow-hidden p-5 rounded-2xl border transition-all duration-300 hover:-translate-y-1 hover:shadow-lg cursor-default text-center
                ${darkMode ? 'bg-white/3 border-white/8 hover:bg-white/6 hover:border-white/15' : 'bg-white border-gray-100 hover:border-gray-200 shadow-sm hover:shadow-md'}`}>
              <div className="absolute top-0 left-0 right-0 h-0.5 rounded-t-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"
        ...
        <div className="mt-14 relative animate-fade-in-up" style={{ animationDelay: '0.6s' }}>
          ...
          <div className={`relative rounded-3xl border overflow-hidden ${darkMode ? 'bg-gray-900/80 border-white/8' : 'bg-white border-gray-200 shadow-xl'}`}>
            ...
            <div className={`flex items-center gap-2 px-4 py-3 border-b ${darkMode ? 'border-white/5 bg-gray-900/50' : 'border-gray-100 bg-gray-50'}`}>
              ...
              <div className={`flex-1 mx-4 px-3 py-1 rounded-lg text-xs font-mono ${darkMode ? 'bg-gray-800 text-gray-400' : 'bg-gray-200 text-gray-500'}`}>
                rfid-attendance.vercel.app
              </div>
              <div className="flex gap-1.5 items-center">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                <span className={`text-xs font-medium ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>Live</span>
            ...
            {/* Live-updating dashboard content */}
            <div className="p-4 md:p-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                {[
                  { label: 'Total Students', val: liveStats.students,     color: '#0ea5e9', flashKey: 'students',     suffix: '' },
                  ...
                  {[72, 85, 78, 90, 83, 88, liveStats.rate].map((h, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      <div className="w-full rounded-t-lg transition-all duration-500"
                        style={{ height: `${Math.max(8, h)}%`, background: i === 6 ? 'linear-gradient(to top, #0ea5e9, #7c3aed)' : darkMode ? '#1e293b' : '#e2e8f0' }} />
                      <span className={`text-xs ${darkMode ? 'text-gray-600' : 'text-gray-400'}`}>{['M','T','W','T','F','S','S'][i]}</span>
                    </div>
                  ))}
                </div>
            ...
            {features.map((f, i) => (
              <button key={i} onClick={() => setActiveFeature(i)}
                className={`w-full text-left p-5 rounded-2xl border transition-all duration-300
                  ${activeFeature === i
                    ? darkMode ? 'bg-white/6 border-white/15 shadow-lg' : 'bg-white border-gray-200 shadow-lg'
                    : darkMode ? 'border-white/4 hover:bg-white/4 hover:border-white/10' : 'border-transparent hover:bg-white hover:border-gray-100'
                  }`}>
                <div className="flex items-center gap-3 mb-1">
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-300
                    ${activeFeature === i ? `bg-gradient-to-br ${f.gradient}` : darkMode ? 'bg-white/5' : 'bg-gray-100'}`}>
                    <f.icon size={15} className={activeFeature === i ? 'text-white' : darkMode ? 'text-gray-400' : 'text-gray-500'} />
                  </div>
                  <div>
                    <p className={`text-xs font-semibold uppercase tracking-wider ${activeFeature === i ? darkMode ? 'text-sky-400' : 'text-sky-600' : darkMode ? 'text-gray-500' : 'text-gray-400'}`}>{f.label}</p>
      ...
      {/* ─── HOW IT WORKS — clean single-column vertical timeline ─── */}
      <section id="how-it-works" className={`relative py-20 ${darkMode ? 'bg-white/2' : 'bg-white'}`} style={{ zIndex: 1 }}>
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-14">
            <p className={`text-xs font-bold uppercase tracking-[0.2em] mb-3 ${darkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>Process</p>
          ...
          {/* Vertical timeline — single column, icon on left, line through it */}
          <div className="relative">
            {/* The continuous vertical line — runs from top icon center to bottom icon center */}
            <div
              className={`absolute top-8 bottom-8 w-px ${darkMode ? 'bg-gradient-to-b from-sky-500 via-violet-500 to-emerald-500' : 'bg-gradient-to-b from-sky-300 via-violet-300 to-emerald-300'}`}
              style={{ left: '31px' }}
            />
            ...
            <div className="space-y-6">
              {steps.map((item, i) => (
                <div key={i} className="relative flex gap-5 items-start group">
                  ...
                  <div
                    className={`relative z-10 flex-shrink-0 w-16 h-16 rounded-2xl flex items-center justify-center shadow-lg bg-gradient-to-br ${item.gradient} transition-transform duration-300 group-hover:scale-105`}>
                    <item.icon size={24} className="text-white" />
                    {/* Step badge */}
                    <div
                      className={`absolute -top-2 -right-2 w-6 h-6 rounded-full text-xs font-black flex items-center justify-center border-2 ${darkMode ? 'bg-gray-900' : 'bg-white'}`}
                  ...
                  {/* Card */}
                  <div className={`flex-1 p-5 rounded-2xl border transition-all duration-300 group-hover:-translate-y-0.5 group-hover:shadow-md
                    ${darkMode ? 'bg-white/3 border-white/6 hover:border-white/12' : 'bg-slate-50 border-gray-100 hover:border-gray-200 hover:bg-white'}`}>
                    <p className={`text-xs font-bold uppercase tracking-wider mb-1`} style={{ color: item.accent }}>
                      Step {item.step}
                    </p>
                    <h3 className={`text-base font-black mb-1.5 ${darkMode ? 'text-white' : 'text-gray-900'}`}>{item.title}</h3>
                    <p className={`text-sm leading-relaxed ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{item.desc}</p>
      ...
      <section id="faq" className="relative max-w-3xl mx-auto px-4 sm:px-6 py-20" style={{ zIndex: 1 }}>
        ...
        <div className="space-y-2">
          {faqs.map((f, i) => (
            <div key={i}
              className={`rounded-2xl border overflow-hidden transition-all duration-300 ${activeFaq === i ? darkMode ? 'bg-white/5 border-white/12' : 'bg-white border-gray-200 shadow-md' : darkMode ? 'border-white/5 hover:border-white/10' : 'border-gray-100 hover:border-gray-200 bg-white'}`}>
              <button onClick={() => setActiveFaq(activeFaq === i ? null : i)} className="w-full flex items-center justify-between p-5 text-left">
                <span className={`font-bold text-sm ${darkMode ? 'text-white' : 'text-gray-900'}`}>{f.q}</span>
                <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ml-3 transition-all duration-300 ${activeFaq === i ? 'bg-sky-500' : darkMode ? 'bg-white/5' : 'bg-gray-100'}`}>
                  {activeFaq === i ? <ChevronUp size={14} className="text-white" /> : <ChevronDown size={14} className={darkMode ? 'text-gray-400' : 'text-gray-500'} />}
      ...
      {/* ─── CTA ─── */}
      <section className="relative max-w-4xl mx-auto px-4 sm:px-6 py-16 text-center" style={{ zIndex: 1 }}>
        <div className={`relative p-10 md:p-14 rounded-3xl overflow-hidden border ${darkMode ? 'border-white/8' : 'border-transparent'}`}
          style={{ background: darkMode ? 'linear-gradient(135deg, rgba(14,165,233,0.08), rgba(124,58,237,0.08))' : 'linear-gradient(135deg, #eff6ff, #f5f3ff)' }}>
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute top-0 left-1/4 w-64 h-64 rounded-full blur-3xl opacity-20" style={{ background: 'radial-gradient(circle, #0ea5e9, transparent)' }} />
            <div className="absolute bottom-0 right-1/4 w-64 h-64 rounded-full blur-3xl opacity-20" style={{ background: 'radial-gradient(circle, #7c3aed, transparent)' }} />
          </div>
          <div className="relative z-10">
            ...
            <h2 className={`text-4xl md:text-5xl font-black tracking-tight mb-4 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
              Start tracking smarter
            </h2>
            <p className={`text-base mb-8 max-w-lg mx-auto ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              Sign in to access live dashboards, parent portals, and one-click reports — all powered by your RFID readers.
            </p>
            <button onClick={() => setShowModal(true)}
              className="inline-flex items-center gap-2.5 px-8 py-4 text-base font-bold text-white rounded-xl transition-all duration-200 hover:scale-105 active:scale-95 shadow-xl shadow-sky-500/30 group"
              style={{ background: 'linear-gradient(135deg, #0ea5e9 0%, #7c3aed 100%)' }}>
              Access Portal <ArrowRight size={18} className="transition-transform duration-200 group-hover:translate-x-1" />
            </button>
            <div className="flex flex-wrap items-center justify-center gap-6 mt-8 text-sm">
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-8">
            <div>
              ...
              <p className={`text-xs leading-relaxed ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
                Modern attendance management for Philippine schools. Built on proven hardware and cloud infrastructure.
              </p>
            </div>
            <div>
              <p className={`text-xs font-bold uppercase tracking-widest mb-3 ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>Quick Links</p>
              <div className="space-y-2">
                {['Features', 'How It Works', 'FAQ'].map(l => (
                  <button key={l}
                    ...
                    className={`block text-xs font-medium transition-colors ${darkMode ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-900'}`}>{l}</button>
                ))}
              </div>
            </div>
            <div>
            ...
            <div className="p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button type="submit" disabled={loading}
                  className="w-full py-3.5 text-white font-bold rounded-xl transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-60 flex items-center justify-center gap-2 shadow-lg shadow-sky-500/25"
                  style={{ background: 'linear-gradient(135deg, #0ea5e9, #7c3aed)' }}>
                  {loading ? <><Loader2 size={16} className="animate-spin" /> Signing in…</> : <><LogIn size={16} /> Sign In</>}
                </button>
              </form>
